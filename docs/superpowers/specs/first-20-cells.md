...
- **AMBER (code merged this session; goes green on owner action / real traffic):**
  ...
  - **20 — Northwind provisioning (#145):** idempotent, dry-run-first `provisionTenant` runbook + CLI + dormant-seam inventory. **Execution (Northwind live as tenant #2) is owner-run** — real Clerk org / CO licenses / Twilio+10DLC / QB+Stripe.
...
- **Remaining work (handoff `docs/superpowers/specs/2026-07-06-closeout-handoff.md`):** cell 16 send-cadence + templates · cell 12r versioned cost-sheet persistence + price-guard rebase · cell 18 reporting surfaces. Plus owner-action cards: Northwind launch (cell 20 execution), 10DLC carrier registration (cell 6 green), financing vendor (cell 11 adapter), **production Clerk instance** (Northwind go-live blocker — prod currently runs Clerk **development** keys `generous-gannet-21.clerk.accounts.dev`; the console warns about strict usage limits. Move to a production Clerk instance before Northwind go-live. Changing auth config was out of scope for the 2026-07-06 lockout hotfix; filed here so it can't be forgotten).
- **P0 HOTFIX (2026-07-06) — onboarding lockout:** existing tenants with real jobs/leads were redirected to `/onboarding` on every route because the `(app)` layout gate reads `settings.onboarding.requiredCompletedAt` and it was never backfilled when the gate shipped, and the wizard's skip navigated away without ever writing it. Fix: migration **0059** backfills the flag for every tenant with ≥1 job/lead, the wizard skip now calls the lifecycle writer then lands on `/`, and a new **`onboarding.no_lockout`** invariant (registry task **214**, bound into the nightly sweep) reds any tenant with work + a null flag so a gate regression pages instead of silently locking customers out.
...
|---|---|---|---|
| 1 | **Per-tenant timezone** — `tenant.timezone`, all crons + customer-facing times read it; kill every hardcoded TZ | Northwind (Denver) can't onboard without it; fixes GMT-in-SMS bug | No "GMT" or wrong-TZ string can reach a customer (invariant exists and passes) |
| 2 | **Task registry schema + seed** — task_registry, tenant_task_config, job_task, verification_run, task_health; seed 212 + approved Phase 0/16–23 tasks | The spine; scoreboard source | 212+ rows seeded; RLS tests green; per-tenant configs for Bloom + Northwind |
...
| 20 | **Northwind provisioning runbook — scripted, then executed** — numbers, templates, price book, licenses seed, timezone, registry config; Northwind goes live on it | The acquisition thesis in miniature: if Northwind onboarding isn't scripted, no acquisition ever will be | Northwind live as tenant #2 via the script; provisioning time logged as the baseline to beat |

---

## Explicitly NOT in the first 20 (and why that's correct)

**Storm Sentinel + PostGrid mail, Strike/Stalk Lists, golden roofs** — wave 2 (cells 21+). Demand gen before the ops spine means leads the machine fumbles. Northwind's launch leads come from what needs no build: LSA + GBP with existing 4-minute speed-to-lead, referrals, and your own network. The sentinel enters the moment cells 1–8 are green — likely before storm season peaks.
