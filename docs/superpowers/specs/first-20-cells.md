...
- **Remaining work (handoff `docs/superpowers/specs/2026-07-06-closeout-handoff.md`):** cell 16 send-cadence + templates · cell 12r versioned cost-sheet persistence + price-guard rebase · cell 18 reporting surfaces. Plus owner-action cards: Alta launch (cell 20 execution), 10DLC carrier registration (cell 6 green), financing vendor (cell 11 adapter), **production Clerk instance** (Alta go-live blocker — prod currently runs Clerk **development** keys `generous-gannet-21.clerk.accounts.dev`; the console warns about strict usage limits. Move to a production Clerk instance before Alta go-live. Changing auth config was out of scope for the 2026-07-06 lockout hotfix; filed here so it can't be forgotten).
