...
| 5 | **Coverage Map + Job Ledger (read-only UI)** — the scoreboard on Today/Agents; per-job task checklist with evidence links; Sage cites ledger rows | Trust surface: proof-of-execution visible; "is X done?" answerable | Map renders live health; Sage answers a done-question with cited evidence |
