...
3. **Address→job selection rule:** normalize the address (lowercase, trim, collapse whitespace, standardize common abbreviations), find the tenant's `property` by normalized address; pick the **most recent job on that property whose stage is not `complete`/`lost`**; if none open, the newest job; if no property/job, **unmatched**.
