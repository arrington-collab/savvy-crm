...
const PUBLIC = [/^\/$/, /^\/api\/health$/, /^\/intake\//, /^\/crew\//, /^\/api\/leads$/, /^\/api\/canvass\/(login|contract|reps|knocks|eod|territories|dossier|geocode|storms|certificate|company|scoreboard)$/, /^\/api\/twilio\//, /^\/api\/inngest$/, /^\/api\/stripe\/webhook$/, /^\/api\/docuseal\/webhook$/, /^\/api\/companycam\/webhook$/, /^\/api\/clerk\/webhook$/, /^\/api\/voice\/vapi$/, /^\/sign-in/, /^\/sign-up/, /^\/select-org$/, /^\/b\//, /^\/book\//, /^\/status\//];
