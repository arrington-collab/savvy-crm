...
import { sendRetailSmsTouch } from "./retail-cadence";

// Compliance follow-up: retailCloseoutCadence's SMS drip touches previously
// called sender.sendSms directly inside the Inngest step.run callback,
// bypassing the global contact_suppression list. sendRetailSmsTouch is the
// extracted, deps-injected send site (mirroring dunning.ts's sendDunningStep
// / homeowner-crew-notify.ts's sendCrewTouch) so it's directly unit-testable
// without an Inngest test harness.
...
async function seedTenantCustomer(custOverrides: Partial<typeof customer.$inferInsert> = {}) {
  const [t] = await adminDb.insert(tenant).values({
    name: "Retail Co", publicKey: `pk-${crypto.randomUUID()}`, clerkOrgId: `org-${crypto.randomUUID()}`,
  }).returning();
  const tenantId = t!.id;
  ...
  const [p] = await adminDb.insert(property).values({ tenantId, customerId: c!.id, address: "1 Retail Ln" }).returning();
...
describe("sendRetailSmsTouch — retail cadence SMS routed through guardedSms", () => {
  it("sends to a consented, reachable customer and logs the communication", async () => {
    ...
    const res = await sendRetailSmsTouch({
      tenantId, jobId, customerId, phone, smsOptOut: false, emailOptOut: false,
      smsConsentAt: new Date("2026-01-01"), body: "Your balance is $100. Pay here: https://x.test/b/abc",
    }, deps);
    ...
    expect(smsRow!.body).toBe("Your balance is $100. Pay here: https://x.test/b/abc");
  });
  ...
  it("globally suppressed customer → not texted, mock sender not called, logged body reflects the block (not the real body)", async () => {
    ...
    const res = await sendRetailSmsTouch({
      tenantId, jobId, customerId, phone: "+16025550555", smsOptOut: false, emailOptOut: false,
      smsConsentAt: new Date("2026-01-01"), body: "Your balance is $100. Pay here: https://x.test/b/abc",
    }, deps);
    ...
    expect(smsRow!.body).not.toBe("Your balance is $100. Pay here: https://x.test/b/abc");
  });
