    ...
    await step.run("send", async () => {
      ...
      if (ctx.email && !ctx.emailOptOut) {
        try {
          ...
          await emailSender.sendEmail({ to: ctx.email, from: process.env.EMAIL_FROM ?? "noreply@example.com", subject: "Your roofing materials arrive tomorrow", html: `<p>${touch.body}</p><p><a href="${link}">Track your project</a></p>` });
        } catch { /* fail-soft */ }
