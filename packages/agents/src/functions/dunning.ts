...
export const dunningRun = inngest.createFunction(
  ...
  { event: "invoice/sent" },
  async ({ event, step }) => {
    const { tenantId, invoiceId } = event.data as { tenantId: string; invoiceId: string };
    ...
    for (const s of steps) {
      ...
      await step.run(`send-${s.stepNum}`, async () =>
        withTenant(tenantId, async (tx) => {
          ...
          // Flip invoice to "overdue" on the SMS escalation step when still "sent".
          ...
          if (s.channel === "email") {
            ...
            let _emailId = "mock";
            try {
              ({ id: _emailId } = await resendEmail.sendEmail({
                to: cust.email,
