...
import {
  ...
  startInspectionByAddress,
  ingestInspectionMedia,
  completeInspection,
  getInspectionProgress,
  getActiveInspectionForLead,
} from "./inspection.js";
...
/** Start and narrow: slice-1 tests always start against a valid seeded lead. */
async function mustStart(tenantId: string, leadId: string): Promise<{ created: boolean; inspectionId: string }> {
  const res = await startInspectionForLead({ tenantId, leadId });
...
describe("startInspectionForLead", () => {
  it("creates an in_progress inspection scoped to the lead's property", async () => {
    ...
    const res = await mustStart(ctx.tenantId, ctx.leadId);
  ...
  it("is idempotent: a second start while one is in_progress returns the existing inspection", async () => {
    const ctx = await seedContext();
    const first = await mustStart(ctx.tenantId, ctx.leadId);
    const second = await mustStart(ctx.tenantId, ctx.leadId);
...
describe("startInspectionByAddress (BloomCam start-capture event)", () => {
  it("resolves the newest OPEN lead by normalized address and starts its inspection", async () => {
    const ctx = await seedContext();
    // A lost lead on the same property must be passed over for the open one.
    await adminDb.update(lead).set({ status: "lost" }).where(eq(lead.id, ctx.leadId));
    const [openLead] = await adminDb.insert(lead)
      .values({ tenantId: ctx.tenantId, customerId: ctx.customerId, propertyId: ctx.propertyId, source: "web", status: "qualified" })
      .returning();

    const res = await startInspectionByAddress({ tenantId: ctx.tenantId, address: "1 Roof Record Rd." });
    expect("inspectionId" in res).toBe(true);

    const [row] = await adminDb.select().from(inspection).where(eq(inspection.id, (res as { inspectionId: string }).inspectionId));
    expect(row!.leadId).toBe(openLead!.id);
    expect(row!.status).toBe("in_progress");
  });

  it("returns no_lead_match when no open lead exists at the address", async () => {
    const ctx = await seedContext();
    await adminDb.update(lead).set({ status: "lost" }).where(eq(lead.id, ctx.leadId));
    const res = await startInspectionByAddress({ tenantId: ctx.tenantId, address: "1 Roof Record Rd" });
    expect(res).toEqual({ error: "no_lead_match" });
  });
});
...
describe("ingestInspectionMedia", () => {
  it("creates the zone on first media for a zone_key and links the photo to it", async () => {
    const ctx = await seedContext();
    const started = await mustStart(ctx.tenantId, ctx.leadId);
  ...
  it("appends a capture note to the zone's inspector notes exactly once (replay-safe)", async () => {
    const ctx = await seedContext();
    const started = await mustStart(ctx.tenantId, ctx.leadId);
    const docId = await seedPhotoDoc(ctx.tenantId, ctx.leadId);
    const input = {
      ...
      note: "3 lifted shingles at the ridge line",
    };
    await ingestInspectionMedia(input);
    await ingestInspectionMedia(input); // outbox replay

    const [zone] = await adminDb.select().from(inspectionZone)
      .where(and(eq(inspectionZone.inspectionId, started.inspectionId), eq(inspectionZone.zoneKey, "north_slope")));
    const notes = zone!.inspectorNotes as { text: string; source: string }[];
    expect(notes).toHaveLength(1);
    expect(notes[0]!.text).toBe("3 lifted shingles at the ridge line");
    expect(notes[0]!.source).toBe("capture");
  });
...
describe("getActiveInspectionForLead", () => {
  it("returns live progress for the lead's capture-window inspection, null otherwise", async () => {
    const ctx = await seedContext();
    expect(await getActiveInspectionForLead({ tenantId: ctx.tenantId, leadId: ctx.leadId })).toBeNull();

    const started = await mustStart(ctx.tenantId, ctx.leadId);
    const docId = await seedPhotoDoc(ctx.tenantId, ctx.leadId);
    await ingestInspectionMedia({ tenantId: ctx.tenantId, inspectionId: started.inspectionId, zoneKey: "attic", zoneLabel: "Attic", zoneKind: "attic", documentId: docId });

    const active = await getActiveInspectionForLead({ tenantId: ctx.tenantId, leadId: ctx.leadId });
    expect(active!.inspectionId).toBe(started.inspectionId);
    expect(active!.zones).toHaveLength(1);

    // pending_approval still shows (approval window), approved does not.
    await completeInspection({ tenantId: ctx.tenantId, inspectionId: started.inspectionId });
    expect((await getActiveInspectionForLead({ tenantId: ctx.tenantId, leadId: ctx.leadId }))!.status).toBe("pending_approval");
    await adminDb.update(inspection).set({ status: "approved" }).where(eq(inspection.id, started.inspectionId));
    expect(await getActiveInspectionForLead({ tenantId: ctx.tenantId, leadId: ctx.leadId })).toBeNull();
  });
});
...
describe("RLS", () => {
  it("cross-tenant reads of inspections return nothing", async () => {
    ...
    const started = await mustStart(a.tenantId, a.leadId);
